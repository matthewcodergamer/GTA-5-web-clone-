...
function buildWorld(){
  ...
  for(const geo of[new THREE.PlaneGeometry(16,120),new THREE.PlaneGeometry(120,16)]){const r=new THREE.Mesh(geo,roadMat);r.rotation.x=-Math.PI/2;r.position.y=.006;world.add(r);shootables.push(r);}
  const geo=new THREE.BoxGeometry(1,1,1),colors=[0xc4c0b4,0xaebbb6,0xcbbdab,0xa8b2a7],blocks=[[-24,-24,11,10,9],[-39,-23,11,16,7],[24,-25,13,11,12],[40,-26,12,12,8],[-24,24,12,13,7],[-40,26,11,11,11],[25,25,12,12,9],[41,27,12,15,13],[-24,-42,12,12,14],[24,-42,12,12,8],[-24,42,12,12,10],[24,42,12,12,15]];
  blocks.forEach(([x,z,w,d,h],i)=>{const b=new THREE.Mesh(geo,new THREE.MeshStandardMaterial({color:colors[i%colors.length],roughness:.89}));b.scale.set(w,h,d);b.position.set(x,h/2,z);b.receiveShadow=true;world.add(b);shootables.push(b);collisionBoxes.push(new THREE.Box3(new THREE.Vector3(x-w/2-.5,0,z-d/2-.5),new THREE.Vector3(x+w/2+.5,h,z+d/2+.5)));});
  ...
  for(const[x,z]of[[-9,-22],[9,-30],[-10,29],[11,38],[-31,9],[31,-10],[-44,8],[44,-8]]){const tr=new THREE.Mesh(trunkGeo,trunkMat);tr.position.set(x,.6,z);const cr=new THREE.Mesh(crownGeo,leafMat);cr.scale.set(1,1.25,1);cr.position.set(x,1.65,z);world.add(tr,cr);shootables.push(tr,cr);}
}
...
function installControls(){
  ...
  $('#qualityToggle')?.addEventListener('click',()=>{quality=(quality+1)%2;const balanced=!quality;renderer.setPixelRatio(Math.min(devicePixelRatio||1,balanced?1.5:1));renderer.shadowMap.enabled=balanced;$('#qualityLabel').textContent=balanced?'Balanced':'Fast';toast(balanced?'Balanced quality':'Fast mode');});
